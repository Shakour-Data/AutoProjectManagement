...
graph TD
    ...
    C --> D[برنامه‌ریزی تسک‌پییوفتاشب]ه
    D --> E[شروع گزویش‌سیی]تلد شدخوکار
    E --> F[سیستم تغییرات را نظارت می
    F --> G[کامیت خودکاره آستانه]
    G --> H[سیستیروغیسایتشفاتنظت می‌کن
    H --> I[خلاصه پایان روز]
```تولید‌ایپیفت
