...
همه متدها پیام‌های خطای مناسب و کدهای خروجی برای یکپارچه‌سازی با ابزارهای دیگر برمی‌گردانند.

## وابستگی‌ها

- **Click**: ایجاد رابط خط فرمان
- **Rich**: قالب‌بندی ترمینال و نوارهای پیشرفت
- **Requests**: ارتباط HTTP API
- **Croniter**: تجزیه عبارت cron (اختیاری)

## فایل‌های پیکربندی

- `JSonDataBase/OutPuts/dashboard_config.json`: تنظیمات داشبورد
- `JSonDataBase/OutPuts/dashboard_schedules.json`: زمان‌بندی گزارش‌ها

## ویژگی‌های عملکرد

- **زمان راه‌اندازی**: ~2 ثانیه (شبیه‌سازی شده)
- **زمان پاسخ API**: وابسته به عملکرد بک‌اند
- **استفاده حافظه**: حداقل برای عملیات CLI
- **استفاده شبکه**: درخواست‌های HTTP به API داشبورد

## ملاحظات امنیتی

- به طور پیش‌فرض از localhost برای امنیت استفاده می‌کند
- نیاز به احراز هویت برای داشبورد محلی ندارد
- فایل‌های پیکربندی به صورت محلی ذخیره می‌شوند
- هیچ داده حساسی در خروجی CLI افشا نمی‌شود
